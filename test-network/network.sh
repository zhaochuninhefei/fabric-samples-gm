...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm gcbaas-gm/fabric-tools:$IMAGETAG peer version | sed -ne 's/ Version: //p' | head -1)
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(docker run --rm gcbaas-gm/fabric-ca:$CA_IMAGETAG fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
